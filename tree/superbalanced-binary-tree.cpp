...
/** @file superbalanced-binary-tree.cpp
 *  @brief 
 *
...
class BinaryTree {
    ...
    *   @return void
    */
    Node<T>* _addHelperFunc(Node<T> *root, T val) {
        if (root->data > val) {
            if (!root->left) {
                root->left = new Node<T>(val);
                return root->left;
            } else {
                return _addHelperFunc(root->left, val);
            }
        } else {
            if (!root->right) {
                ...
                return _addHelperFunc(root->right, val);
            }
        }
    }
    ...
    */
    Node<T> *addNode(T val) {
        if (root) {
            return this->_addHelperFunc(root, val);
        } else {
            root = new Node<T>(val);
            return root;
        }
...
template <class T>
void superbalanced_binary_tree(Node<T>* node){

}

template <class T>
void superbalanced_binary_tree_helper(Node<T>* node ){
